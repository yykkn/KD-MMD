...
from DataLoader import create_loaders, DiGCN
...
model2 = GraphTransformerNetwork(in_channels=num_features, hidden_channels=64, out_channels=1).to(device)
...
for epoch in range(1, 201):
    ...
    if test_acc1 > best_acc1:
        best_acc1 = test_acc1
        torch.save(model1.state_dict(), 'model1_best_BGL.pth')
    if test_acc2 > best_acc2:
        best_acc2 = test_acc2
        torch.save(model2.state_dict(), 'model2_best_BGL.pth')

    print(f'Epoch: {epoch:03d}, Model1 Loss: {loss1:.4f}, Model1 Train Acc: {train_acc1:.4f}, Model1 Test Acc: {test_acc1:.4f}')
    print(f'Epoch: {epoch:03d}, Model2 Loss: {loss2:.4f}, Model2 Train Acc: {train_acc2:.4f}, Model2 Test Acc: {test_acc2:.4f}')
    print(f'Best Model1 Test Acc: {best_acc1:.4f}')
    print(f'Best Model2 Test Acc: {best_acc2:.4f}')

test_acc = test(model1, test_loader)
print(f'Model1 Final Test Acc: {test_acc:.4f}')

test_acc = test(model2, test_loader)
print(f'Model2 Final Test Acc: {test_acc:.4f}')