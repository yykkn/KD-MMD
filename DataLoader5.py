...
def load_data(data_name, seed=1213):
    np.random.seed(seed)
    torch.manual_seed(seed)

    print(f"Loading dataset {data_name}...")

    if os.path.exists(f"{DATA_PATH}/{data_name}/Raw/"):
        print(f"Found dataset {data_name}")
        dataset_raw = ParseDataset(root=DATA_PATH, name=data_name)
    else:
        raise NotImplementedError(f"Dataset {data_name} not found")

    dataset_list = [data for data in dataset_raw]

    # 打印所有样本的标签分布
    all_labels = [data.y.item() for data in dataset_list]
    unique_labels, counts_labels = np.unique(all_labels, return_counts=True)
    print(f"All samples label distribution: {dict(zip(unique_labels, counts_labels))}")

    # 按照要求划分训练集、混合训练集和测试集
    normal_samples = [i for i, data in enumerate(dataset_list) if data.y.item() == 0]
    anomaly_samples = [i for i, data in enumerate(dataset_list) if data.y.item() == 1]

    # 正常样本的训练集选择50%的正常样本
    normal_train_indices, test_indices = train_test_split(normal_samples, train_size=0.5, random_state=seed)

    # 混合样本的训练集选择30%正常和50%异常（与正常样本的训练集不重叠）
    normal_train_indices_mixed = [i for i in normal_samples if i not in normal_train_indices]
    mixed_train_indices = normal_train_indices_mixed.copy()
    mixed_train_indices += [i for i in anomaly_samples if np.random.random() < 0.5]  # 50%异常样本

    # 根据索引创建数据集
    normal_train_dataset = [dataset_list[idx] for idx in normal_train_indices]
    mixed_train_dataset = [dataset_list[idx] for idx in mixed_train_indices]
    test_dataset = [dataset_list[idx] for idx in test_indices]

    # 检查并平衡分布（如果需要）
    unique_train_normal, counts_train_normal = np.unique([data.y.item() for data in normal_train_dataset],
                                                         return_counts=True)
    unique_train_mixed, counts_train_mixed = np.unique([data.y.item() for data in mixed_train_dataset],
                                                       return_counts=True)
    unique_test, counts_test = np.unique([data.y.item() for data in test_dataset], return_counts=True)
    print(f"Normal Training set label distribution: {dict(zip(unique_train_normal, counts_train_normal))}")
    print(f"Mixed Training set label distribution: {dict(zip(unique_train_mixed, counts_train_mixed))}")
    print(f"Test set label distribution: {dict(zip(unique_test, counts_test))}")

    return normal_train_dataset, mixed_train_dataset, test_dataset, dataset_raw

def create_loaders(data_name, batch_size=64, seed=1213):
    normal_train_dataset, mixed_train_dataset, test_dataset, dataset_raw = load_data(data_name, seed=seed)

    # 打印加载后的数据集信息
    labels = np.array([data.y.item() for data in normal_train_dataset])
    label_dist = ['%d' % (labels == c).sum() for c in [0, 1]]
    print("Normal Training set: Number of graphs: %d, Class distribution %s" % (len(normal_train_dataset), label_dist))

    labels = np.array([data.y.item() for data in mixed_train_dataset])
    label_dist = ['%d' % (labels == c).sum() for c in [0, 1]]
    print("Mixed Training set: Number of graphs: %d, Class distribution %s" % (len(mixed_train_dataset), label_dist))

    labels = np.array([data.y.item() for data in test_dataset])
    label_dist = ['%d' % (labels == c).sum() for c in [0, 1]]
    print("Test set: Number of graphs: %d, Class distribution %s" % (len(test_dataset), label_dist))

    # 创建 DataLoader
    Loader = DataLoader
    num_workers = 0

    normal_train_loader = Loader(normal_train_dataset, batch_size=batch_size, shuffle=True, pin_memory=True,
                                 num_workers=num_workers)
    mixed_train_loader = Loader(mixed_train_dataset, batch_size=batch_size, shuffle=True, pin_memory=True,
                                num_workers=num_workers)
    test_loader = Loader(test_dataset, batch_size=batch_size, shuffle=False, pin_memory=True, num_workers=num_workers)

    return normal_train_loader, mixed_train_loader, test_loader, normal_train_dataset[0].num_features, dataset_raw
# =============================================================================
...
class DiGCN(nn.Module):
    def __init__(self, nfeat, nhid, nlayer, dropout=0, bias=False, out_dim=1, **kwargs):
        super(DiGCN, self).__init__()
        self.conv1 = DIGCNConv(nfeat, nhid)
        self.conv2 = DIGCNConv(nhid, nhid)
        self.fc = nn.Linear(nhid, out_dim)  # 添加线性层以输出标量概率值
    ...
    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        edge_attr = data.edge_attr if hasattr(data, 'edge_attr') else None  # 检查是否存在 edge_attr 属性
        ...
        for g in range(data.num_graphs):
            emb = x[data.batch == g]
            emb = torch.mean(emb, dim=0)  # 对图中的节点嵌入求平均
            emb = self.fc(emb)  # 使用线性层将嵌入转换为标量
            emb_list.append(emb)

        return torch.stack(emb_list)  # 将列表转换为张量
